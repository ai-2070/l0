...
const canonicalSpec = JSON.parse(
  readFileSync(join(__dirname, "../tests/fixtures/canonical-spec.json"), "utf8"),
);

// Extract event specs from canonical spec
const eventSpecs = canonicalSpec.monitoring.observabilityEvents.events as Record<
  string,
...
describeIf(hasOpenAI)(
  "Event Signature Validation (Live LLM)",
  () => {
    ...
    it(
      ...
      async () => {
        ...
        expect(
          validation.eventCounts["GUARDRAIL_PHASE_START"],
        ).toBeGreaterThan(0);
        expect(validation.eventCounts["GUARDRAIL_PHASE_END"]).toBeGreaterThan(
          0,
        );
      },
      LLM_TIMEOUT,
    ...
    describe("comprehensive event coverage", () => {
      it(
        ...
        async () => {
              ...
              rules: [
                {
                  id: "length-check",
                  check: (ctx) => ({
                    passed: ctx.content.length < 1000,
                  }),
                },
              ],
            },
          ...
          console.log("Captured event types:", Object.keys(validation.eventCounts));
