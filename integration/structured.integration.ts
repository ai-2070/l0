...
import { structured, structuredArray, structuredObject } from "../src/structured";
...
describeIf(hasOpenAI)("Structured Output Integration", () => {
  describe("Basic Structured Output", () => {
    it("should parse simple object schema", async () => {
      ...
      const result = await structured({
        ...
        stream: () =>
          streamText({
            ...
            prompt: "Generate a person with name and age as JSON. Use any values.",
          }),
        autoCorrect: true,
      });
    ...
    it("should parse nested object schema", async () => {
      ...
      const result = await structured({
        ...
        stream: () =>
          streamText({
            model: openai("gpt-4o-mini"),
            prompt: "Generate a user object with nested user (name, email) and active boolean as JSON.",
          }),
        autoCorrect: true,
      });
    ...
    it("should parse array of objects", async () => {
      ...
      const result = await structuredArray(itemSchema, {
        stream: () =>
          streamText({
            model: openai("gpt-4o-mini"),
            prompt: "Generate a JSON array with 3 items, each having id (number) and title (string).",
          }),
        autoCorrect: true,
      });
  ...
  describe("Object Helper", () => {
    it("should work with structuredObject helper", async () => {
      const result = await structuredObject(
        ...
        },
        {
          stream: () =>
            streamText({
              model: openai("gpt-4o-mini"),
              prompt: "Generate a city info object with city, country, and population as JSON.",
            }),
          autoCorrect: true,
        }
      );
    ...
    it("should handle markdown-wrapped JSON", async () => {
      ...
      const result = await structured({
        ...
        stream: () =>
          streamText({
            model: openai("gpt-4o-mini"),
            prompt: 'Return this exact text including the backticks: ```json\n{"value": "test"}\n```',
          }),
        autoCorrect: true,
      });
    ...
    it("should handle enums", async () => {
      ...
      const result = await structured({
        ...
        stream: () =>
          streamText({
            model: openai("gpt-4o-mini"),
            prompt: 'Generate JSON with status (one of: active, inactive, pending) and priority (one of: low, medium, high).',
          }),
        autoCorrect: true,
      });
    ...
    it("should handle optional fields", async () => {
      ...
      const result = await structured({
        ...
        stream: () =>
          streamText({
            model: openai("gpt-4o-mini"),
            prompt: 'Generate JSON with a "required" string field. Optionally include an "optional" field.',
          }),
        autoCorrect: true,
      });
    ...
    it("should handle arrays with constraints", async () => {
      ...
      const result = await structured({
        ...
        stream: () =>
          streamText({
            model: openai("gpt-4o-mini"),
            prompt: "Generate JSON with a 'tags' array containing 2-3 string tags.",
          }),
        autoCorrect: true,
      });
  ...
  describe("Telemetry", () => {
    it("should include structured output telemetry", async () => {
