...
describeIf(hasOpenAI)("Mastra AI Integration", () => {
  describe("mastraStream", () => {
    it("should stream from Mastra agent", async () => {
  ...
  describe("wrapMastraStream", () => {
    it("should wrap Mastra stream result", async () => {
      const agent = createTestAgent();
    ...
    it("should include usage when enabled", async () => {
      const agent = createTestAgent();
      const streamResult = await agent.stream("Hi");

      let doneEvent: any;
      for await (const event of wrapMastraStream(streamResult, { includeUsage: true })) {
        if (event.type === "done") {
          doneEvent = event;
        }
      }
  ...
  describe("extractMastraText", () => {
    it("should extract full text from stream", async () => {
      const agent = createTestAgent();
  ...
  describe("With Guardrails", () => {
    it("should apply guardrails to Mastra output", async () => {
      const agent = createTestAgent();
  ...
  describe("With Monitoring", () => {
    it("should collect telemetry from Mastra stream", async () => {
      const agent = createTestAgent();
  ...
  describe("With Fallback", () => {
    it("should fall back from failed Mastra stream", async () => {
      const agent = createTestAgent();
      ...
      const result = await l0({
        ...
        fallbackStreams: [
          mastraStream(agent, "Say 'fallback worked'"),
        ],
      });
